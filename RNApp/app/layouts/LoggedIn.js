...
import { FormIcon } from '../routes/Form';
...
class LoggedInLayout extends React.Component {
  ...
  renderTabItem(title, initialRoute, Icon) {
    ...
    if (initialRoute.showNavigationBar !== false) {
      sceneStyle.push({ paddingTop: 64 , backgroundColor:'#008000'});
    }
  ...
  render() {
    return (
      <TabNavigator tabBarStyle={{overflow: 'hidden' }}>
        {this.renderTabItem('Home', Routes.getHomeRoute(), HomeIcon)}
        {this.renderTabItem('Form', Routes.getFormRoute(), FormIcon)}
